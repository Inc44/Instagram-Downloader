	...
	return {
		...
		current: Object.freeze(
		{
			...
			set highlights(value)
			{
				...
				downloadStoryPhotos('highlights')
					.then(data =>
					{
						renderMedias(data);
...
(() =>
{
	...
	function initUI()
	{
		document.body.appendChild(createElement(`<div class="display-container hide">
	...
	function handleEvents()
	{
		...
		function setTheme()
		{
			const isDarkMode = localStorage.getItem('igt') === null ? window.matchMedia('(prefers-color-scheme: dark)')
				.matches : localStorage.getItem('igt') === 'dark';
			if (isDarkMode)
			{
		...
		function pauseVideo()
		{
			...
			{
				DISPLAY_CONTAINER.querySelectorAll('video')
					.forEach(video =>
					{
						video.pause();
					});
		...
		function toggleSelectMode()
		{
			if (TITLE_CONTAINER.classList.contains('multi-select'))
			{
				...
				DISPLAY_CONTAINER.querySelectorAll('.overlay')
					.forEach(element =>
					{
		...
		window.addEventListener('keydown', (e) =>
		{
			...
			if (Array.from(SELECT_EVENT_KEYS[0])
				.includes(e.key) && !DISPLAY_CONTAINER.classList.contains('hide'))
			{
				return handleSelectAll();
			}
			if (Array.from(SELECT_EVENT_KEYS[1])
				.includes(e.key) && !DISPLAY_CONTAINER.classList.contains('hide'))
			{
		...
		{
			DISPLAY_CONTAINER.querySelectorAll('img , video')
				.forEach(media =>
				{
					media.src = media.src;
				});
	...
	function run()
	{
		document.querySelectorAll('.display-container, .download-button')
			.forEach(node =>
			{
				node.remove();
			});
