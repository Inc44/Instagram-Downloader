...
async function saveZip()
{
	...
	const medias = Array.from(document.querySelectorAll('.overlay.checked'))
...
async function handleDownload()
{
	...
	if (TITLE_CONTAINER.classList.contains('multi-select') && !DISPLAY_CONTAINER.classList.contains('hide') && option === 'none' && totalItemChecked.length !== 0)
	{
		return saveZip();
	}
...
function renderMedias(data)
{
	...
	data.medias.forEach(item =>
	{
		...
		const itemDOM = new DOMParser()
			.parseFromString(ITEM_TEMPLATE, 'text/html')
